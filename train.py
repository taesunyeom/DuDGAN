...
def setup_training_loop_kwargs(
    ...
    if cfg is None: #training setting
        cfg = 'auto'
    ...
    args.D_kwargs = dnnlib.EasyDict(class_name='training.networks.Discriminator', block_kwargs=dnnlib.EasyDict(), mapping_kwargs=dnnlib.EasyDict(), D_epilogue_kwargs=dnnlib.EasyDict())
    args.C_kwargs = dnnlib.EasyDict(class_name='training.networks.Classifier', block_kwargs=dnnlib.EasyDict(), mapping_C_kwargs=dnnlib.EasyDict(), C_epilogue_kwargs=dnnlib.EasyDict())
    args.G_kwargs.synthesis_kwargs.channel_base = args.D_kwargs.channel_base = args.C_kwargs.channel_base = int(spec.fmaps * 32768) # modified
    args.G_kwargs.synthesis_kwargs.channel_max = args.D_kwargs.channel_max = args.C_kwargs.channel_max = 512 # modified
    args.G_kwargs.mapping_kwargs.num_layers = spec.map
    args.G_kwargs.synthesis_kwargs.num_fp16_res = args.D_kwargs.num_fp16_res = args.C_kwargs.num_fp16_res = 4 # enable mixed-precision training # modified
    args.G_kwargs.synthesis_kwargs.conv_clamp = args.D_kwargs.conv_clamp = args.C_kwargs.conv_clamp = 256 # clamp activations to avoid float16 overflow # modified
    args.D_kwargs.D_epilogue_kwargs.mbstd_group_size = spec.mbstd # modified
    args.C_kwargs.C_epilogue_kwargs.mbstd_group_size = spec.mbstd # added

    args.G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=spec.lrate, betas=[0,0.99], eps=1e-8) #for optimization
    args.D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=spec.lrate, betas=[0,0.99], eps=1e-8)
    args.C_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.AdamW', lr=spec.lrate * 0.1, betas=[0.9,0.99], eps=1e-8, weight_decay=0.01) # added
    ...
    diffusion_specs_D = dict(beta_schedule=beta_schedule, beta_start=beta_start, beta_end=beta_end,
                           t_min=t_min, t_max=t_max, noise_std=noise_sd,
                           aug=aug, ada_maxp=ada_maxp, ts_dist=ts_dist)
    
    diffusion_specs_C = dict(beta_schedule=beta_schedule, beta_start=beta_start, beta_end=beta_end,
                           t_min=t_min, t_max_c=t_max_c, noise_std=noise_sd,
                           aug=aug, ada_maxp=ada_maxp, ts_dist=ts_dist) #init에서 선언한 변수들

    desc += f"-ts_dist-{ts_dist}-image_aug{aug}-noise_sd{noise_sd}"
    args.diffusion_D_kwargs = dnnlib.EasyDict(class_name='training.diffusion.Diffusion_D', **diffusion_specs_D)
    args.diffusion_C_kwargs = dnnlib.EasyDict(class_name='training.diffusion_C.Diffusion_C', **diffusion_specs_C)
    
    #contrastive_specs = dict(temperature=temperature, contrast_mode=contrast_mode, base_temperature=base_temperature)
    #args.contrastive_kwargs = dnnlib.EasyDict(class_name='training.contrastive_loss.SupConLoss', **contrastive_specs)
    # ----------------------------------
    ...
    if fp32:
        args.G_kwargs.synthesis_kwargs.num_fp16_res = args.D_kwargs.num_fp16_res = args.C_kwargs.num_fp16_res = 0
        args.G_kwargs.synthesis_kwargs.conv_clamp = args.D_kwargs.conv_clamp = args.C_kwargs.conv_clamp = None #수정

    if nhwc is None:
        nhwc = False
    assert isinstance(nhwc, bool)
    if nhwc:
        args.G_kwargs.synthesis_kwargs.fp16_channels_last = args.D_kwargs.block_kwargs.fp16_channels_last = args.C_kwargs.block_kwargs.fp16_channels.last = True #수정
...
@click.option('--t_max_c', help='for C',type=int, default=300)
