...
def q_sample(x_0, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, noise_type='gauss', noise_std=1.0):
    if noise_type == 'gauss':
        noise = torch.randn_like(x_0, device=x_0.device) * noise_std * 0.5
...
class Diffusion_C(torch.nn.Module):
    def __init__(self,
        beta_schedule='linear', beta_start=1e-4, beta_end=2e-2,
        t_min=10, t_max_c=1000, noise_std=0.05,
    ...
    def update_T(self):
        ...
        t = np.clip(int(self.t_min + t_adjust), a_min=self.t_min, a_max=self.t_max_c)
